...
_META_COLS = (
    "id, doc_number, doc_type, filename, mime, uploaded_at, status, confidence, data, issues"
)
...
def insert_document(rec: dict[str, Any], file_bytes: bytes) -> None:
    ...
    with _connect() as conn:
        conn.execute(
            ...
            ":status, :confidence, :data, :issues, :file)",
            {
                "id": rec["id"],
...
def list_documents() -> list[dict[str, Any]]:
    """All records, newest first (no file blobs)."""
    with _connect() as conn:
        rows = conn.execute(
            f"SELECT {_META_COLS} FROM documents ORDER BY uploaded_at DESC, rowid DESC"
        ).fetchall()
