...
MODEL_NAME = "qwen/qwen/qwen3-vl-4b"
...
def _build_prompt(doc_type: DocumentType, markdown_content: str | None = None) -> str:
    """Compose the instruction text, embedding the target JSON schema and optionally the markdown text."""
    schema = json.dumps(Document.model_json_schema(), indent=2)
    source_instruction = "Read the attached document text" if markdown_content else f"Read the attached {doc_type.value} image"
    
    prompt = (
        "You are a precise document-extraction engine. "
        f"{source_instruction} and extract its fields.\n\n"
...
def extract_document(image_b64: str, doc_type: DocumentType) -> Document:
    ...
    return _execute_llm_request(payload, doc_type)


def extract_document_text(markdown_content: str, doc_type: DocumentType) -> Document:
    """Run extraction for a text/markdown document and return a validated Document.
        ...
        "messages": [
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": _build_prompt(doc_type, markdown_content=markdown_content)}
                ],
            }
        ],
    }
...
def _execute_llm_request(payload: dict, doc_type: DocumentType) -> Document:
    """Helper to send the payload to LM Studio and parse the response."""
    try:
